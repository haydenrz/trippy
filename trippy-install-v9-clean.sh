...
# Trippy v9: Immich-style web UI route-tour generator for Proxmox LXC
# Adds stop-based clustering, stop radius, stop review/editing, and lasso grouping.
#
# Run on Proxmox host:
#   bash trippy-install-v9.sh
#
# Optional:
#   CTID=106 STORAGE=local-lvm BRIDGE=vmbr0 bash trippy-install-v9.sh
...
printf "${CYAN}${BOLD}Trippy v9 Clean Installer${RESET}\n"
printf "${DIM}Immich-style trip tours • stop grouping • preview suite • efficient renderer${RESET}\n\n"
...
printf "${DIM}Use CTID=106 bash install.sh if you want a different target.${RESET}\n\n"
sleep 1
...
step "Clean install target"
if pct status "$CTID" >/dev/null 2>&1; then
  printf "${YELLOW}!${RESET} CTID ${BOLD}$CTID${RESET} exists. Performing clean rebuild.\n"
...
app = FastAPI(title="Trippy", version="0.9.0")
...
echo
printf "${CYAN}${BOLD}v9 features${RESET}\n"
