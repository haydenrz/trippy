...
# Trippy v10: Immich-style web UI route-tour generator for Proxmox LXC
# Adds stop-based clustering, stop radius, stop review/editing, and lasso grouping.
#
# Run on Proxmox host:
#   bash trippy-install-v10.sh
#
# Optional:
#   CTID=106 STORAGE=local-lvm BRIDGE=vmbr0 bash trippy-install-v10.sh
...
cat <<'LOGO'
      ⛰️  ◌  TRIPPY  ◌  🧭
  ┌──────────────────────────────┐
  │   your journey, mapped weird │
  └──────────────────────────────┘
          ✦ route • stops • photos
LOGO

printf "${CYAN}${BOLD}Trippy v10 Clean Installer${RESET}\n"
...
pct set "$CTID" --description "🧭 Trippy v10
...
app = FastAPI(title="Trippy", version="1.0.0")
...
echo
printf "${CYAN}${BOLD}v10 features${RESET}\n"
