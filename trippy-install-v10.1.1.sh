...
# Trippy v10.1.1: Immich-style web UI route-tour generator for Proxmox LXC
# Adds stop-based clustering, stop radius, stop review/editing, and lasso grouping.
#
# Run on Proxmox host:
#   bash trippy-install-v10.1.1.sh
#
# Optional:
#   CTID=106 STORAGE=local-lvm BRIDGE=vmbr0 bash trippy-install-v10.1.1.sh
...
printf "${CYAN}${BOLD}Trippy v10.1.1 Clean Installer${RESET}\n"
...
pct set "$CTID" --description "🧭 Trippy v10.1.1
...
app = FastAPI(title="Trippy", version="1.1.1")
...
// Trippy v10.1.1 UI behavior upgrades
(function(){{
  function ready(fn){{ if(document.readyState!=='loading') fn(); else document.addEventListener('DOMContentLoaded',fn); }}
  window.TRIPPY_VERSION='v10.1.1';
  ready(() => {{
    if(!document.querySelector('.versionBadge')){{
      const v=document.createElement('div'); v.className='versionBadge'; v.textContent='v10.1.1'; document.body.appendChild(v);
    }}
...
pct exec "$CTID" -- bash -lc "grep -q 'TRIPPY_UI_V1011_DIRECT' /opt/trippy/backend/main.py && grep -q 'Present Journey' /opt/trippy/backend/main.py" >/dev/null 2>&1 || {
  printf "${RED}ERROR:${RESET} UI smoke check failed. Frontend markers missing.\n"
  exit 1
}
...
echo
printf "${CYAN}${BOLD}v10.1.1 features${RESET}\n"
