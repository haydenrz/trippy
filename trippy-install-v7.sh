...
# Trippy v7: Immich-style web UI route-tour generator for Proxmox LXC
# Adds stop-based clustering, stop radius, stop review/editing, and lasso grouping.
#
# Run on Proxmox host:
#   bash trippy-install-v7.sh
#
# Optional:
#   CTID=106 STORAGE=local-lvm BRIDGE=vmbr0 bash trippy-install-v7.sh
...
if pct status "$CTID" >/dev/null 2>&1; then
  NEW_CONTAINER=0
  echo "[1/8] CTID $CTID already exists. Updating existing container instead of creating a new one."
  if ! pct status "$CTID" | grep -q "status: running"; then
    echo "Starting existing CT $CTID..."
    pct start "$CTID"
    sleep 3
  fi
else
  echo "[1/8] Finding/downloading Debian 12 LXC template..."
  ...
  echo "[2/8] Creating LXC $CTID ($HOSTNAME)..."
...
DEBIAN_FRONTEND=noninteractive apt-get install -y \
  ...
  ffmpeg exiftool nginx unzip jq build-essential
curl -fsSL https://deb.nodesource.com/setup_22.x | bash -
DEBIAN_FRONTEND=noninteractive apt-get install -y nodejs
'

echo "[4/8] Writing app files..."
...
app = FastAPI(title="Trippy", version="0.7.0")
...
<div class="app">
  <aside>
    <h1>Trippy</h1>
    <button class="primary" onclick="newImmich()">New Immich Tour</button>
...
<div id="setupModal" style="display:none;position:fixed;inset:0;z-index:20;background:rgba(0,0,0,.72);align-items:center;justify-content:center;padding:22px">
  <div style="width:min(720px,96vw);background:#111820;border:1px solid #31506a;border-radius:22px;padding:24px;box-shadow:0 0 60px rgba(0,200,255,.18)">
    ...
    <p class="small">Insert your Immich API key so Trippy can read GPS metadata. In Immich, go to Account Settings → API Keys, create a key for Trippy, and include <b>asset.read</b>.</p>
...
<div id="accountModal" style="display:none;position:fixed;inset:0;z-index:21;background:rgba(0,0,0,.72);align-items:center;justify-content:center;padding:22px">
  <div style="width:min(720px,96vw);background:#111820;border:1px solid #31506a;border-radius:22px;padding:24px;box-shadow:0 0 60px rgba(0,200,255,.18)">
    ...
    <p class="small">Update the API key here any time. Required permission: <b>asset.read</b>.</p>
    <label>Immich URL</label>
...
<script>
let project=null, map=null, routeLayer=false, activeStop=null;
...
async function loadProjects(){
  ...
  projects.innerHTML=list.map(p=>`<div onclick="openProject('${p.id}')"><b>${p.name}</b><br><span class="small">${p.count} GPS assets • ${p.stops||0} stops</span></div>`).join('');
}
...
function renderGallery(){
  ...
  gallery.innerHTML=project.assets.map((a,i)=>`
    <div class="tile ${a.selected?'selected':''} ${activeIds.has(a.asset_id)?'instop':''}" onclick="toggle(${i})">
...
function renderStops(){
  ...
  stops.innerHTML=project.stops.map((s,i)=>`
    <div class="stop ${s.stop_id===activeStop?'active':''}" onclick="activeStop='${s.stop_id}'; renderStops(); renderGallery(); drawRoute();">
      <b>${s.name||('Stop '+(i+1))}</b>
...
function toggle(i){project.assets[i].selected=!project.assets[i].selected; renderGallery(); drawRoute(); updateStats();}
function selectAll(){if(!project)return; project.assets.forEach(a=>a.selected=true); hydrate();}
function selectNone(){if(!project)return; project.assets.forEach(a=>a.selected=false); hydrate();}
...
async function save(){
  ...
  project.settings={title:tourTitle.value||project.name,duration_min:Number(duration.value||12),mode:mode.value,pace:pace.value,stop_radius_m:Number(stopRadius.value||200),show_stop_slides:showStopSlides.checked};
...
async function createImmich(){
  ...
  const body={
    name:`Immich Tour ${immichStart.value} to ${immichEnd.value}`,
...
echo "[5/8] Installing Python/Node dependencies..."
pct exec "$CTID" -- bash -lc "
set -e
cd $APP_DIR/backend
python3 -m venv venv
...
pct exec "$CTID" -- bash -lc "systemctl daemon-reload && systemctl enable --now trippy"

echo "[7/8] Waiting for IP..."
sleep 3
IP="$(pct exec "$CTID" -- bash -lc "hostname -I | awk '{print \$1}'" | tr -d '\r')"

echo "[8/8] Done."
echo
echo "Trippy v7 ready:"
echo "  CTID:     $CTID"
echo "  Hostname: $HOSTNAME"
echo "  URL:      http://${IP}:${PORT}"
echo
echo "Default root password: $PASSWORD"
echo "Change it after install: pct enter $CTID && passwd"
echo
echo "v7 features:"
echo "  - Immich-style projects/gallery/map editor"
echo "  - Date range import from Immich"
echo "  - Upload-based GPS media import"
echo "  - Stop radius auto-clustering"
echo "  - Density-first stop location selection"
echo "  - Stop list review"
echo "  - Move stop to current map center"
echo "  - Recenter stop from grouped photos"
