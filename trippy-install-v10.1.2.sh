...
# Trippy v10.1.2: Immich-style web UI route-tour generator for Proxmox LXC
# Adds stop-based clustering, stop radius, stop review/editing, and lasso grouping.
#
# Run on Proxmox host:
#   bash trippy-install-v10.1.2.sh
#
# Optional:
#   CTID=106 STORAGE=local-lvm BRIDGE=vmbr0 bash trippy-install-v10.1.2.sh
...
printf "${CYAN}${BOLD}Trippy v10.1.2 Clean Installer${RESET}\n"
...
pct set "$CTID" --description "🧭 Trippy v10.1.2
...
app = FastAPI(title="Trippy", version="1.1.2")
...
// Trippy v10.1.2 UI behavior upgrades
(function(){{
  function ready(fn){{ if(document.readyState!=='loading') fn(); else document.addEventListener('DOMContentLoaded',fn); }}
  window.TRIPPY_VERSION='v10.1.2';
  ready(() => {{
    if(!document.querySelector('.versionBadge')){{
      const v=document.createElement('div'); v.className='versionBadge'; v.textContent='v10.1.2'; document.body.appendChild(v);
    }}
...
# v10.1.2: patch the ACTUAL served frontend file, not backend/main.py.
...
pct exec "$CTID" -- bash -lc "grep -q 'TRIPPY_UI_V1012_STYLE' /opt/trippy/frontend/index.html && grep -q 'Present Journey' /opt/trippy/frontend/index.html && grep -q 'trippyLogo' /opt/trippy/frontend/index.html" >/dev/null 2>&1 || {
  printf "${RED}ERROR:${RESET} UI smoke check failed. Frontend markers missing from /opt/trippy/frontend/index.html.\n"
  exit 1
}
...
echo
printf "${CYAN}${BOLD}v10.1.2 features${RESET}\n"
...
printf "  ${CYAN}•${RESET} Safer project thumbnail proxy for new projects\n"printf "  ${CYAN}•${RESET} v10.1.2 directly patches /opt/trippy/frontend/index.html
"
