...
# Trippy v8: Immich-style web UI route-tour generator for Proxmox LXC
# Adds stop-based clustering, stop radius, stop review/editing, and lasso grouping.
#
# Run on Proxmox host:
#   bash trippy-install-v8.sh
#
# Optional:
#   CTID=106 STORAGE=local-lvm BRIDGE=vmbr0 bash trippy-install-v8.sh
...
app = FastAPI(title="Trippy", version="0.8.0")
...
<script>
let project=null, map=null, routeLayer=false, activeStop=null, galleryFilterStop=null, focusedAsset=null;
...
function renderStops(){
  ...
  stops.innerHTML=project.stops.map((s,i)=>`
    <div class="stop ${s.stop_id===activeStop?'active':''}" onclick="focusStop('${s.stop_id}')">
      <b>${s.name||('Stop '+(i+1))}</b>
